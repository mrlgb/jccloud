...
import edu.hfuu.jccloud.model.samples.SampleSZ09;
import edu.hfuu.jccloud.model.samples.SampleSZ09Adapter;
 ...
 */
public class SZ09_Dynamic extends BaseFragment {
    private  String title= StringConsts.SR05B;
    private ArrayList<SampleSZ09> mDataSet;
    private SampleSZ09Adapter mAdapter;
    Realm realm;

    @Bind(R.id.myRecyclerViewSZ09)
    RecyclerView mRecyclerView;

    @Bind(R.id.edtAddresssSZ09)
    EditText edtLocation;
    @Bind(R.id.edtBarCodeSZ09)
    EditText edtBarCode;
    @Bind(R.id.btnSelectNewLocationSZ09)
    Button btnSelectLocation;

    @Bind(R.id.inputLayoutTimeSZ09)
    TextInputLayout inputTime;
    @Bind(R.id.inputTimePickerSZ09)
    EditText edtTime;
    @Bind(R.id.btnDynamicAddSZ09)
    Button btnAdd;
    @Bind(R.id.btnDynamicDeleteSZ09)
    Button btnDel;
    @Bind(R.id.btnDynamicSaveSZ09)
    Button btnSave;
    @Bind(R.id.btnDynamicSubmitSZ09)
    Button btnSubmit;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.sz09_dynamic, container, false);
...
//                            Toast.makeText(getContext(), "click RecyclerView"+position , Toast.LENGTH_SHORT).show();
        ...
        mAdapter = new SampleSZ09Adapter(mDataSet, getContext());
            ...
            public void onClick(View v) {
                if (validateData()) {
                    ...
                    if (!codeInList(bcode) && !bcode.isEmpty()) {
                        ...
                        SampleSZ09 sample = new SampleSZ09("Sample" );//name
                        String uuid = UUID.randomUUID().toString();
                        sample.setId(uuid);//id
                        sample.setLocation(edtLocation.getText().toString());
                        sample.setSampleStartTime(edtTime.getText().toString());
...
//                    showMessage("没有删除项！");
    ...
    private void addItem2SampleDb(final SampleSZ09 sample) {
        realm.beginTransaction();
        SampleSZ09 s = realm.createObject(SampleSZ09.class,sample.getBarCode()); // 创建新对象
...
//        s.setBarCode(sample.getBarCode());
    ...
    private void removeItemInSampleDb(final String bcode) {
        SampleSZ09 code = realm.where(SampleSZ09.class)
    ...
    private void initDataSetFromDB() {
        RealmResults<SampleSZ09> results = realm.where(SampleSZ09.class)
                .findAll();
//        RealmResults<SampleSZ09> sortedAscending  = results.sort("index");
        showMessage(title+ SampleDataUpdated);
        if (results.size() > 0) {
//            showMessage("可用样本个数:" + results.size());
            for (int i = 0; i < results.size(); i++) {
                SampleSZ09 item = results.get(i);
                int id = mDataSet.size();
                SampleSZ09 sample = new SampleSZ09("Sample" + id);//name
...
//        showMessage(used ? "Register Barcode:" + code : "Free Barcode: " + code);
    ...
    public void updateDetails(int position) {
        ...
        SampleSZ09 sa = mDataSet.get(position);
        edtBarCode.setText(sa.getBarCode());
        edtLocation.setText(sa.getLocation());
        edtTime.setText(sa.getSampleStartTime());
    }
